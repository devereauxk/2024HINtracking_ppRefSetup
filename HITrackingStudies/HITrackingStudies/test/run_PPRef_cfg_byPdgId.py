...
cmsRun run_PPRef_cfg_byPdgId.py sample="MC_RecoDebug" n=100 usePixelTrks=False  runOverStreams=False
...
from ppref import ppref_mc_recodebug as ppref
if options.sample == "MC_Reco_AOD":
    from ppref import ppref_mc_reco_aod as ppref
if options.sample == "Data_Reco_AOD":
    from ppref import ppref_data_reco_aod as ppref
if options.sample == "Data_Reco_AOD" and options.runOverStreams==True :
    from ppref import ppref_data_reco_aod_streams as ppref
if options.sample == "Data_MiniAOD":
    from ppref import ppref_data_miniaod as ppref
if options.sample == "Data_miniAOD" and options.runOverStreams==True :
    from ppref import ppref_data_miniaod_streams as ppref
if options.sample == "MC_MiniAOD":
    from ppref import ppref_mc_miniaod as ppref
...
process.source = cms.Source("PoolSource",
                            ...
                            fileNames =  cms.untracked.vstring(ppref),
...
process.HITrackCorrections_byPdgId.applyVertexZCut = True
process.HITrackCorrections_byPdgId.doVtxReweighting = True
process.HITrackCorrections_byPdgId.dopthatWeight = True
...
#process.HITrackCorrections_byPdgId.ptErrMax = 0.1 # no cut basically
...
process.HITrackCorrections_byPdgId.vtxWeightParameters = cms.vdouble(0.895491, -0.0124438, 0.00369163, -6.87949e-06)
###
...
#forest style analyzers (anaTrack module) (not affected by HITrackCorrections_byPdgId code)
...
#process.hltMB.HLTPaths = ["HLT_HIUPC_*"] # for 2024 UPC
